...
<li class="payout-node border-bottom py-2" data-shop-id="{{ $node['shop_id'] }}"
    data-children-url="{{ $node['has_children'] ? route('admin.payout.network.children', ['shop' => $node['shop_id']]) . '?year=' . $year . '&month=' . $month : '' }}">
    <div class="d-flex align-items-center gap-2 flex-wrap payout-node-row">
        @if($node['has_children'])
            <button type="button" class="btn btn-sm btn-link p-0 payout-expand text-secondary me-1"
        ...
        <div class="d-flex align-items-center gap-3 text-nowrap small bg-light px-3 py-1 rounded-pill">
            ...
            <span class="text-muted">{{ __('Ph1') }}: <span class="fw-semibold text-primary">{{ number_format($node['phase1_amount'], 2) }}</span></span>
            <span class="text-muted">{{ __('Ph2') }}: <span class="fw-semibold text-info">{{ number_format($node['phase2_amount'], 2) }}</span></span>
            <span class="fw-bold text-success fs-6">{{ number_format($node['total_payout'], 2) }}</span>
        </div>
    </div>
    @if($node['has_children'])
