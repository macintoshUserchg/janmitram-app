                            ...
                            <DialogPanel
                                ...
                                <div class="bg-white p-5 sm:p-8 relative" :class="master.langDirection==='rtl' ? 'text-right' : 'text-left'">
                                    ...
                                    <form @submit.prevent="loginFormSubmit()">
                                        ...
                                        <!-- default credentials -->
                                        <button v-if="master.app_environment === 'local'" type="button" @click="applyDefaultCredentials()"
                                            class="mt-4 w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-left hover:border-primary hover:bg-primary/5 transition-all duration-200"
                                            ...
                                            <div class="text-xs text-slate-500 mb-1 flex items-center justify-between gap-2">
                                                <span>{{ $t('Use default credentials') }}</span>
                                                <ClipboardDocumentIcon class="w-4 h-4 text-slate-500" />
                                            </div>
                                            <div class="text-sm text-slate-700">
                                                <span class="font-medium">{{ $t('Email') }}:</span> {{ defaultLoginCredentials.phone }}
                                            </div>
                                            <div class="text-sm text-slate-700">
                                                <span class="font-medium">{{ $t('Password') }}:</span> {{ defaultLoginCredentials.password }}
                                            </div>
                                        </button>
